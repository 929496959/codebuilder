...
$.extend($.fn.form.methods, {
	...
	save: function (jq, g) {
		...
		function _saveCkList(name, data) {
			var rr = $(jq).find('span[id="' + name + '"].checkBoxListExt');
			if (rr.length == 1) {
				data[name] = "";
				$.each($('input[type="checkbox"]', rr), function(i, r) {
					if ($(r).prop('checked')) {
